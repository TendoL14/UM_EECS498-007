...
class TwoLayerNet(object):
    ...
    def __init__(
        ...
        ###################################################################
        # Replace "pass" statement with your code
        W1 = torch.randn(input_dim, hidden_dim).to(device=device) * weight_scale
        b1 = torch.zeros(hidden_dim).to(device=device)
        W2 = torch.randn(hidden_dim, num_classes).to(device=device) * weight_scale
        b2 = torch.zeros(num_classes).to(device=device)
        self.params["W1"] = W1
        self.params["b1"] = b1
        self.params["W2"] = W2
        self.params["b2"] = b2
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            self.params[f"W{i+1}"] = (
                torch.randn((dims[i], dims[i + 1]), dtype=self.dtype, device=device)
                * weight_scale
            )
            self.params[f"b{i+1}"] = (
                torch.randn((dims[i + 1],), dtype=self.dtype, device=device)
                * weight_scale
            )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            if i + 1 != self.num_layers:
                next_layer, cur_cache = Linear_ReLU.forward(
                    cur, self.params[f"W{i+1}"], self.params[f"b{i+1}"]
                )
            else:
                next_layer, cur_cache = Linear.forward(
                    cur, self.params[f"W{i+1}"], self.params[f"b{i+1}"]
                )
            hidden.append(next_layer)
            hidden_cache.append(cur_cache)
            cur = cur.clone()

            if self.use_dropout:
                dropout_layer, dropout_cache = Dropout.forward(cur, self.dropout_param)
                dropouts.append(dropout_layer)
                dropouts_cache.append(dropout_cache)
                cur = cur.clone()
